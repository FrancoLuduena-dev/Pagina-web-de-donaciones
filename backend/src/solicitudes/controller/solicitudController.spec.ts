...
describe('SolicitudController', () => {
  ...
  describe('metadata del controller', () => {
    it('usa AuthGuard y StatusGuard a nivel controller', () => {
      expect(obtenerMetadata(GUARDS_METADATA, SolicitudController)).toEqual([
        AuthGuard,
        StatusGuard,
      ]);
    });

    it('requiere usuarios activos para acceder a sus rutas', () => {
      expect(obtenerMetadata(ESTADOS_KEY, SolicitudController)).toEqual([
